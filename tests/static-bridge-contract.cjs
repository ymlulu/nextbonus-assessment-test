...
for(const method of ['getVersion','getProductAssessment','getOfferHistory','getOfferTiming','evaluate'])assert.ok(bridge.includes(`'${method}'`),`bridge missing ${method}`);
for(const call of ['AssessmentApiV1.getVersion','AssessmentApiV1.getProductAssessment','AssessmentApiV1.getOfferHistory','AssessmentApiV1.getOfferTiming','AssessmentApiV1.evaluate'])assert.ok(bridge.includes(call),`bridge must delegate to ${call}`);
assert.ok(bridge.includes("'offer_history'"),'bridge runtime must load reviewed offer history from the runtime manifest');
...
console.log(JSON.stringify({channel:'nextbonus-assessment-v1',methods:5,reviewedOfferHistory:true,transport:'static-postMessage',status:'PASS'}));