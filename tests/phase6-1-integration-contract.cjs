...
assert.equal(response.contract_version,'1');assert.equal(response.release_id,runtime.manifest.release_id);assert.equal(response.product_id,productId);assert.equal(response.evaluation_date,evaluationDate);assert.ok(response.decision.recommended_action);assert.ok(response.report.offer_text);assert.equal(response.provenance.release_id,runtime.manifest.release_id);assert.ok(response.offer_history);assert.equal(response.offer_history.offer_timing_id,runtime.products.cards[productId].offer_timing_id);assert.equal(response.offer_history.current.evaluation_date,evaluationDate);assert.ok(Array.isArray(response.offer_history.history));
...
console.log(JSON.stringify({contractVersion:api.contractVersion,product:productId,release:runtime.manifest.release_id,offerHistory:true,browserCoreParity:true,status:'PASS'}));
