...
c=contracts['contracts'];assert c['questions']['authority']=='google_drive_reviewed_docs';assert set(c['questions']['documents'])==ids;assert c['bank_rule_predicates']['authority']=='google_drive_reviewed_workbook_mirror';assert c['frozen_offer_facts']['authority']=='google_drive_reviewed_workbook_mirror'
states={x['state'] for x in orch['runtime_priority']};assert states<=set(templates['final_templates']);assert states<=set(templates['cta'])
assert manifest['engine']['fact_normalizer']=='3.2';assert manifest['engine']['approval_engine']=='2.1';assert manifest['engine']['bank_rule_engine']=='2';assert manifest['engine']['assessment_runtime']=='3.1'
...
print(json.dumps({'runtime_files':len(keys),'repo_contracts':len(lock['repo_contracts']),'rules':len(rules),'products':len(ids),'status':'PASS'},ensure_ascii=False))
