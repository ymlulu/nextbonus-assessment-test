...
function evalMapping(def,answers,tokens){const value=readSpec(answers,def.source);
  ...
  if(def.type==='multi_history'){const arr=readSpec(answers,def.source);if(multiUnknown(arr,tokens))return null;return arr.map(clean).filter(v=>v&&v!==def.none_value)}
