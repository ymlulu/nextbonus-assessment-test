...
function evalMapping(def,answers,tokens){const value=readSpec(answers,def.source);
  ...
  if(def.type==='multi_history'){if(!Array.isArray(value)||value.length===0)return null;if(value.some(x=>isUnknown(x,tokens)))return null;return value.map(clean).filter(v=>v&&v!==def.none_value)}
...
window.FactNormalizerV3={normalize,version:'fact-normalizer-3.1'};
})();
