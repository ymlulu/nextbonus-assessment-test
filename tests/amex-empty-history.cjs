...
for(const card of ['amex_gold','amex_platinum']){
  ...
  assert.equal(norm(card,{Q6:['不确定']}),null,'explicit unsure must be UNKNOWN');
  assert.deepEqual(Array.from(norm(card,{Q6:['都没有']})),[],'explicit none must be clear empty history');
